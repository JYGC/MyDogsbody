...
**The invoices page writes to an external system.** Since `invoice-calendar-sync` (change #7 of the invoice-to-calendar series), syncing invoices can create, update and delete events on a real Google Calendar — the only write this application makes outside its own databases, and the only one that can destroy data it does not own. `DiffInvoicesAgainstCalendarWorkflow.diff` is the pure function the whole guarantee rests on: see its own doc comment, and *Architecture → Errors*, for the two hazards it is built to make structurally impossible rather than conditionally avoided.
...
| Ring | Project | Holds |
| --- | --- | --- |
| **Centre** | `MyDogsbody.Domain` | The pure domain: `Documents/`, `Suppliers/`, `InvoiceTemplates/`, `Invoices/`, `MailAccounts/` and `Calendar/`, each holding `<Area>Types.fs` (constrained types, stage types, the error DU, the dependency function types) and one `<Workflow>Workflow.fs` per workflow. Plus `Result.fs`, its own generic `result` builder. References **no other project**, and a build target in its `.fsproj` fails the build if one is ever added. `Calendar/` breaks the one-file-per-area convention slightly: `InvoiceSyncKey.fs` (the one supplier+reference derivation, compiled ahead of `CalendarTypes.fs` because the latter's `SyncAction`/`LedgerSnapshot` need the type it declares) sits beside `CalendarTypes.fs`, and the events half adds `CalendarDateRangeWorkflow.fs` (mirrors the scan window around today, stretched forward to cover a due date beyond it), `DiffInvoicesAgainstCalendarWorkflow.fs` (`diff` — pure, the heart of `invoice-calendar-sync`; `buildPlan` binds a calendar read before calling it) and `SyncInvoicesToCalendarWorkflow.fs` (`executePlan`) |
| Outer ring | `MyDogsbody.Integrations.Google` (+ `.Database.Models`, C#) | *(real since `credentials-per-provider`; `google-account-integration` made the account/calendar half real too)* `GoogleCredentialTypes.fs` — constrained types `GoogleCredentialSecret` / `GoogleExternalUsername` / `GoogleCredentialId`, and `ValidGoogleCredential` / `ValidGoogleCredentialEdit` / `StoredGoogleCredential`, **in the integration, not the domain** (a credential is not a domain concept). `GoogleDatabaseContext.fs` (context record: `GetCredentialCollection` / `GetAccountCollection` / `GetClientSecretCollection`), `GoogleDatabaseContextModule.fs` (`getDatabaseContext` — uses a **local** `BsonMapper`, not `BsonMapper.Global`, with `TrimWhitespace`/`EmptyStringToNull` off so a secret round-trips byte-for-byte; all three entities warmed), `GoogleCredentialEntityMappers.fs` (the credential-only bottom mapper, no enum-translation pair) and `GoogleEntityMappers.fs` (the account-half bottom mapper, entity ⇄ `RegisteredGoogleAccount` — **this is what gives the integration its first `Domain` reference**), `GoogleCredentialStore.fs` (`getAll` / `insertOne` / `updateOne`) and `GoogleAccountStore.fs` (client secret + account CRUD), both outer-ring shape. `GoogleCredentialDataStore.fs` is a custom `Google.Apis.Util.Store.IDataStore` over the `Credentials` collection (so an OAuth token lands in `Google.db`, never a `FileDataStore` directory). `GoogleAuthorization.fs` is the system-browser + loopback consent flow (`GoogleWebAuthorizationBroker`), plus `loadCredential` (loads a stored token and lets the SDK refresh it silently — never opens a browser) and `removeStoredToken`. `authorise` goes through `authoriseNewAccountWith`, which hands back the token consent wrote if the new-account authorisation then fails. Await its consent flow with `.GetAwaiter().GetResult()`, never `Async.AwaitTask |> Async.RunSynchronously`: the latter surfaces a faulted task as an `AggregateException` that no typed catch matches, and a test fake must return a faulted `Task`, not `raise`, or it will pass over exactly that. `GoogleCalendarClient.fs` binds `listCalendars`, and, since `invoice-calendar-sync`, the events half too — `listEvents` / `createEvent` / `updateEvent` / `deleteEvent`, each in the same outer-ring `...Via (httpClientFactory option) -> ... -> Result<'T, MyDogsbodyException>` shape, with a stable message literal per distinguishable failure (`eventNotFoundMessage`, `eventRejectedPrefix`, `calendarGoneMessage`, alongside the existing `apiNotEnabledPrefix`/`unreachablePrefix`) that `GoogleAccountApiMappers`'s per-operation translators (`toListCalendarEventsError` and three siblings) match on to produce `EventNoLongerExists` / `EventRejected` / `CalendarNoLongerExists`. An integration is an **adapter**: it implements the function types the domain declares |
...
| Composition | `MyDogsbody.Startup` | The composition root. `*ApiMappers.fs` (domain ⇄ UI + error translation, pure) and `*ApiFactory.fs` (`create*Api`, dependencies as parameters) for Supplier / Template / MailAccount / Invoice / ScanWindow / GoogleAccount / **InvoiceSync**; `Startup.fs` (the Logging, Thunderbird and Google LiteDB contexts, the main SQLite context, migration run, `getCurrentTime = DateTime.Now`, shared `handleError`, `registerServices`). `InvoiceApiMappers` owns the shared `InvoiceError` ⇄ `MyDogsbodyException` translation (the ledger, the scan-window and the invoice-sync factories all return `InvoiceError` from their main-database reads). `GoogleAccountApiMappers` splits its inbound translation by which adapter action produced the exception — `toAuthorisationError` / `toListCalendarsError` / `toStoreError`, and, since `invoice-calendar-sync`, one per event operation (`toListCalendarEventsError` / `toCreateCalendarEventError` / `toUpdateCalendarEventError` / `toDeleteCalendarEventError`) — since `NotAuthorised` needs an account id, and `EventNoLongerExists`/`CalendarNoLongerExists` the id being acted on, that only each call site's own arguments carry. `GoogleAccountApiFactory` also carries the four event-operation bindings (`bindListCalendarEvents` and three siblings), in the same public, Google-facing-call-as-parameter shape `bindListCalendars` already established, so `InvoiceSyncApiFactory.createInvoiceSyncApiWith` (and `Contracts/*CalendarEventDependencyContractTests.fs`) can run them over a stubbed `HttpMessageHandler` rather than a copy. Plugs real adapters into workflows and maps between the two error types |
...
| UI | `MyDogsbody.UI.Types` | UI-facing records (`SupplierUiType*`, `SupplierApi`, `TemplateUiType*`, `MailAccountUiType*`, `InvoiceUiType*`, `GoogleAccountUiType`, `CalendarUiType`, `GoogleAccountApi`) and `Modules/` adaptive-state records |
...
- **An integration references `Domain` when it implements a function type the domain declares** — `Integrations.Documents` and `Integrations.Thunderbird` both do, and as of `google-account-integration`, **`Integrations.Google` now does too**: `GoogleEntityMappers.fs` maps to `RegisteredGoogleAccount`, and `GoogleAccountStore`/`GoogleAuthorization`/`GoogleCalendarClient` satisfy `LoadClientSecret`, `SaveClientSecret`, `ListGoogleAccounts`, `SaveGoogleAccount`, `RemoveGoogleAccount`, `AuthoriseAccount`, `ReauthoriseAccount`, `DiscardAuthorisation` and `ListCalendars`. This does not contradict the credential bullet above: a credential is still not a domain concept, and `GoogleCredentialTypes.fs`/`GoogleCredentialStore.fs` still declare and satisfy nothing domain-shaped — only the account/calendar half of the integration references `Domain`. No integration ever references another, `Startup`, or the UI.
...
- Fresh temp file per test — `Data Source={Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid()}.db")};Pooling=False`. **Put `;Pooling=False` on every SQLite connection string a test builds** (the one it hands `MigrationSetup.setupMigrations`, and any raw `new SqliteConnection`), matching `DatabaseContextSetup.createDatabaseContext`. Microsoft.Data.Sqlite pools by default, and a pooled handle keeps the temp file locked on Windows after `Dispose()` — which drove the old harnesses to the process-global `SqliteConnection.ClearAllPools()`, disposing connections other parallel collections were mid-command on (`ObjectDisposedException: 'SQLitePCL.sqlite3'`). With pooling off, `Dispose()` / `use` releases the handle and there is nothing global to clear. `Database/SqliteConnectionPoolingTests` enforces both halves of this: it fails if a harness reintroduces `ClearAllPools`, **and** if any test source line building a `Data Source=` string omits `;Pooling=False`. See `docs/changes/sqlite-pool-flake`.
...
- `createDatabaseContext` constructs a `SqliteConnection` and never disposes it. Dispose it in the test (`GetDatabaseConnection()` hands it back); with `Pooling=False` that releases the file handle, so `try File.Delete path with _ -> ()` afterwards is reliable.
...
- **Never let a test reach `Startup.Startup`.** Its module-level `let` bindings open `Logging.db`, `MyDogsbody.db`, `Thunderbird.db` and `Google.db` (and run its migrations) in the process working directory the moment anything in the module is touched. That is why the composition root is split three ways per API: test `SupplierApiMappers` and `SupplierApiFactory` (and the other API pairs the same way) — all free of module-level I/O and taking their dependencies as parameters — and leave `Startup.fs` alone. Keep any new composition the same shape.
...
- **Dependency function types** are this architecture's published interfaces, so CLAUDE.md's shared-suite rule applies to each one. The failure it catches here: a fake returning shapes the real store never produces, leaving a workflow's unit suite green over code that cannot work in production. Where the real implementation is a network service, the suite runs the composition root's own binding with only the network call stubbed (`GoogleAccountApiFactory.bindListCalendars`, driven by `Contracts/ListCalendarsDependencyContractTests.fs`), never a re-composition written in the test: a copy of `ListCalendars`' error translation there left the production binding untested, and a mis-wired binding passed the whole suite. The same holds for a binding that only network- or browser-gated members use, which no factory test can reach: `Contracts/GoogleAccountDependencyContractTests.fs` runs `GoogleAccountApiFactory`'s `bindSaveGoogleAccount`, `bindDiscardAuthorisation` and its other storage bindings, because with copies there a factory that never saved an account passed the whole suite. The same holds one level up, for an API member that reaches its store only after Google (registering, re-authorising, choosing a calendar): its composition is a public function taking the Google-facing dependency as a parameter (`GoogleAccountApiFactory.registerAccountWith` and the rest), which `createGoogleAccountApi` and `Startup/GoogleAccountApiFactoryTests.fs` call, and the E2E harness too for every one a flow exercises (all but `reauthoriseAccountWith`). Re-composed in the harness instead, a `RegisterAccount` handed a discard that did nothing passed the whole suite. `invoice-calendar-sync` repeats the same pattern for the events half: `Contracts/ListCalendarEventsDependencyContractTests.fs` and its three siblings each drive `GoogleAccountApiFactory.bindListCalendarEvents`/`bindCreateCalendarEvent`/`bindUpdateCalendarEvent`/`bindDeleteCalendarEvent` over stubbed HTTP alongside an in-memory fake, and `Contracts/InvoiceCalendarEventStoreDependencyContractTests.fs` does the same for `MarkSynced`/`ClearSyncRecord`/`LoadAllLedgerKeys` over a real temp SQLite database. `Contracts/InvoiceSyncApiContractTests.fs` is the one place this pattern runs into a real gap rather than closing it: `InvoiceSyncApiFactory.createInvoiceSyncApi` (unlike `createGoogleAccountApi`) has no per-member public composition to call instead, so its shared real/fake suite is scoped to the paths that never reach Google at all (no ready account; a ready-but-unauthorised one) — the same accepted scoping `Contracts/GoogleAccountApiContractTests.fs` already uses for `GetCalendarsFor`/`SetDefaultInvoiceCalendar`, stated there in that file's own header comment. A successful plan/execute is exercised only via `createInvoiceSyncApiWith` in the E2E harness (`InvoiceSyncTestHarness.fs`) and via the fake in the contract suite, not via the real assembled API — left as a named gap for whoever next touches `InvoiceSyncApiFactory.fs`, not a silent one.
...
- **The harnesses exist**, one per E2E area: `E2E/SuppliersTestHarness.fs`, `E2E/MailAccountsTestHarness.fs`, `E2E/InvoicesTestHarness.fs`, `E2E/GoogleAccountsTestHarness.fs`, `E2E/InvoiceSyncTestHarness.fs`. Each subclasses bUnit's `TestContext`, registers `AddMudServices()`, and sets `JSInterop.Mode <- JSRuntimeMode.Loose` — MudBlazor calls into JS for popovers and key interception during render, and none of that is under test. Use `withSuppliersHarness` for the happy paths and `withUnreachableSupplierStoreHarness` for failure paths. (`E2E/BlazorTestHarness.fs` was deleted by `credentials-per-provider` — it had turned out to be entirely credentials-specific.) `InvoiceSyncTestHarness.fs` seeds a real temp `Google.db` with a client secret, a stored token and a ready registered account, and stubs only the Google Calendar HTTP call, against `FakeGoogleCalendar` — a small in-memory stand-in for one calendar's events (list/insert/update/delete, decompressing the GZip-compressed write bodies `Google.Apis`'s client sends by default) that round-trips a create through a later list. It composes `InvoiceSyncApi` via `InvoiceSyncApiFactory.createInvoiceSyncApiWith`, not `createInvoiceSyncApi`: the latter hardwires the production Google client with no stubbing seam, the same way `createGoogleAccountApi` does for `ListCalendars` — `createInvoiceSyncApiWith` is the four-Google-call-as-parameter seam that makes a real end-to-end sync flow testable without duplicating `GetSyncPlan`/`ExecuteSyncPlan`'s own logic in the harness.
...
| `Startup.fs` | database paths, the LiteDB and SQLite contexts, `handleError`, `supplierApi`, `registerServices` | opens `Logging.db`, `MyDogsbody.db`, `Thunderbird.db` and `Google.db` |
...
`MyDogsbody.Database` is the application's main store. `DatabaseContextSetup.createDatabaseContext databaseFilePath` registers Dapper.FSharp's `OptionTypes`, opens a `SqliteConnection` (`Data Source={path};Foreign Keys=True;Pooling=False` — `Foreign Keys` is the connection-string keyword, not a runtime `PRAGMA`, so it self-applies on every open; `Pooling=False` because a pooled handle survives `Dispose()` and keeps the file locked, which is what made temp-database test cleanup fail. It is **not** free, and do not repeat the claim that it is: one `SqliteConnection` *object* is held for the process lifetime, but its underlying handle is opened and closed **per store operation** — explicitly in `SupplierStore`/`TemplateStore`'s `inTransaction`, and by Dapper around every `SelectAsync` on a closed connection — so the pool was amortising real work. Measured on Microsoft.Data.Sqlite 9.0.10 over 2000 open/query/close cycles on one connection object: 0.090 ms per cycle pooled, 0.470 ms unpooled (+0.38 ms, 5.2×). A suppliers page load is two of those cycles, so well under a millisecond — a price knowingly paid to stop the suite failing ~2 runs in 45; see `docs/changes/sqlite-pool-flake`) and returns a `DatabaseContext` record of getters — `GetDatabaseConnection`, plus one `unit -> QuerySource<'T>` per table, each bound to its table name via `table'<Blog> "Blogs"` — and a `Dispose` that closes the connection. Models are F# `[<CLIMutable>]` records in `MyDogsbody.Database.Models`. Same context-record-of-getters shape as the integrations use, so it partially applies into an outer-ring function the same way — and stops at the same boundary: a `QuerySource<'T>` is no more allowed in a domain signature than an `ILiteCollection<T>` is.
...
`Migration_20260810000009_CreateInvoiceCalendarEventsTable.fs` (`invoice-calendar-sync`, change #7 — the last of the series) adds `InvoiceCalendarEvents(Id, InvoiceId FK → Invoices.Id ON DELETE CASCADE, GoogleAccountId, CalendarId, EventId, LastSyncedAt)` with a unique index on `InvoiceId`, plus `MyDogsbody.Database/InvoiceCalendarEventStore.fs` (`markSynced`, `clearSyncRecord`, `loadAllLedgerKeys`, `loadSyncRecords`). Reserved as `20260809000010` in the background record, then renumbered to `20260810000009` as that change's own first task — the reserved number sorted *before* the `invoice-extraction` migrations it has a foreign key to, which would have failed on a fresh database; see `docs/changes/invoice-to-calendar/background.md` → *Migration timestamps*. This table is history, not truth: the calendar itself remains the source of truth for the diff that decides what to create, update or delete; the table only answers "when did we last touch this event, and on whose calendar?"
...
- **Production is not exposed the way the paragraph above implies.** `Startup.fs`'s module-level bindings construct all four contexts (Logging, Thunderbird, Google, plus the main SQLite context) sequentially on one thread, so every warm-up completes before any `Async.Start` thread exists. The race needs two contexts under construction at once, which today only the parallel test suite does.
