...
let ``DocumentPath.create does not require the file to exist`` () =
    // whether a file is there is I/O, and the domain performs none. The adapter finds
...
let ``DocumentSource carries the bytes, name and format it was built from`` () =
    // a plain carrier: an attachment lives inside a multi-GB mbox and must not be
