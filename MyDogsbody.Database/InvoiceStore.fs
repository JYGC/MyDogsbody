...
/// SQLITE_CONSTRAINT_FOREIGNKEY. The primary code, 19, is SQLITE_CONSTRAINT and covers unique and
/// check violations too, so it is the extended one that identifies a foreign-key violation.
[<Literal>]
let private ExtendedResultCodeSqliteReportsForAForeignKeyViolation = 787

/// Rationale: docs/changes/comments-to-names/rationale/MyDogsbody.Database.md - InvoiceStore.fs: isMissingSupplier
let isMissingSupplier (exceptionToExamine: MyDogsbodyException) : bool =
    let rec search (candidate: exn) : bool =
        match candidate with
        | null -> false
        | :? SqliteException as sqlite when
            sqlite.SqliteExtendedErrorCode = ExtendedResultCodeSqliteReportsForAForeignKeyViolation -> true
