...
- [ ] **0.1** Create `MyDogsbody.Domain/MyDogsbody.Domain.fsproj`, `net9.0`, with **no**
      `ProjectReference` and **no** `PackageReference`. Add it to `MyDogsbody.sln`.
      *Outcome:* the centre exists and is empty.
- [ ] **0.2** Add `MyDogsbody.Domain` `ProjectReference` to `MyDogsbody.Tests.fsproj`.
      *Depends on:* 0.1.
- [ ] **0.3** `Tests/Domain/ResultTests.fs` — `result` returns, binds an `Ok`, short-circuits an
      ...
      *Depends on:* 0.2.
- [ ] **0.4** `MyDogsbody.Domain/Result.fs` at the project root, compiled first — `Bind`, `Return`,
      `ReturnFrom`, `Zero`, `Delay`, `Run`. No `TryWith`, no `writeLog`, no error-type annotation.
      *Depends on:* 0.3.
- [ ] **0.5** `Tests/Contracts/DomainIsolationTests.fs` — parse `MyDogsbody.Domain.fsproj` and
...
- [ ] **1.1** `Tests/Domain/Credentials/CredentialsTypesTests.fs` — `CredentialSecret.create`,
      ...
      *Depends on:* 0.4.
- [ ] **1.2** `Tests/Domain/Credentials/AddCredentialWorkflowTests.fs` — Ok path asserting **every
      ...
      *Depends on:* 1.1.
- [ ] **1.3** `Tests/Domain/Credentials/EditCredentialWorkflowTests.fs` — Ok path, every field;
      ...
      *Depends on:* 1.1.
- [ ] **1.4** `Tests/Domain/Credentials/ListCredentialsWorkflowTests.fs` — every field of every
      returned credential; empty store returns `Ok []`; store error propagated as its own case.
      *Depends on:* 1.1.

### Implementation

- [ ] **1.5** `MyDogsbody.Domain/Credentials/CredentialsTypes.fs` — `Infrastructure` DU, the three
      ...
      *Depends on:* 1.1.
- [ ] **1.6** `MyDogsbody.Domain/Credentials/AddCredentialWorkflow.fs` — one public function.
      *Depends on:* 1.2, 1.5.
- [ ] **1.7** `MyDogsbody.Domain/Credentials/EditCredentialWorkflow.fs` — one public function;
      loads, decides not-found, then updates.
      *Depends on:* 1.3, 1.5.
- [ ] **1.8** `MyDogsbody.Domain/Credentials/ListCredentialsWorkflow.fs` — one public function.
      *Depends on:* 1.4, 1.5.

## Phase 2 — Credentials adapter (required)

- [ ] **2.1** Add `MyDogsbody.Domain` `ProjectReference` to
      `MyDogsbody.Integrations.Credentials.fsproj`.
- [ ] **2.2** `Tests/Integrations/Credentials/CredentialStoreTests.fs` (Integration) — fresh temp
      ...
      *Depends on:* 2.1, 1.5.
- [ ] **2.3** `Tests/Integrations/Credentials/CredentialsDatabaseContextModuleTests.fs`
      (Integration) — the getter returns a collection usable against a temp file.
      *Depends on:* 2.1.
- [ ] **2.4** `MyDogsbody.Integrations.Credentials/CredentialEntityMappers.fs` — the **bottom
      ...
      `match`.
      *Depends on:* 2.1.
- [ ] **2.5** `MyDogsbody.Integrations.Credentials/CredentialStore.fs` — `getAll`, `insertOne`,
      ...
      *Depends on:* 2.2, 2.4.
- [ ] **2.6** Delete `Repositories/` and `UseCases/` from `MyDogsbody.Integrations.Credentials`,
      including their `Types/` folders and the four DTO records, and remove them from the `.fsproj`.
      *Depends on:* 2.5.

## Phase 3 — Composition root (required)

- [ ] **3.1** `Tests/Startup/CredentialApiMappersTests.fs` — rewrite for the new types: UI record →
      ...
      *Depends on:* 1.5.
- [ ] **3.2** `Tests/Contracts/ErrorTranslationTests.fs` — every `CredentialError` case maps to the
      intended action and message; an adapter `MyDogsbodyException` maps to the intended domain case.
      *Depends on:* 1.5.
- [ ] **3.3** Add `ActionNames.MyDogsbody.Startup.CredentialApi` — one entry per API operation.
      Resolution 2 in design.md → *Specification conflicts found*.
      *Depends on:* nothing.
- [ ] **3.4** `MyDogsbody.Startup/CredentialApiMappers.fs` — rewrite onto domain types; add
      `toMyDogsbodyException`. Total functions, no module-level bindings.
      *Depends on:* 3.1, 3.2, 3.3.
- [ ] **3.5** `MyDogsbody.Startup/CredentialApiFactory.fs` — bind `CredentialStore` functions to
      ...
      *Depends on:* 3.4, 2.5.
- [ ] **3.6** Rewrite `Tests/Startup/CredentialApiFactoryTests.fs` (Integration) for the new wiring
      ...
      *Depends on:* 3.5.
- [ ] **3.7** Add `MyDogsbody.Domain` and both integration `ProjectReference`s to
      ...
      *Depends on:* 3.5.
- [ ] **3.8** Confirm `MyDogsbody.UI.Portal` and `MyDogsbody.UI.Types` are unchanged by this phase,
...
- [ ] **4.1** `Tests/Domain/Documents/DocumentsTypesTests.fs` — `DocumentPath.create` accepts a
      path, rejects null, empty and whitespace, reason asserted.
      *Depends on:* 0.4.
- [ ] **4.2** `Tests/Domain/Documents/ReadDocumentLinesWorkflowTests.fs` — the four grouping cases
      ...
      *Depends on:* 4.1.
- [ ] **4.3** `MyDogsbody.Domain/Documents/DocumentsTypes.fs` and
      ...
      *Depends on:* 4.1, 4.2.
- [ ] **4.4** Add `MyDogsbody.Domain` `ProjectReference` to `MyDogsbody.Integrations.Pdf.fsproj`.
- [ ] **4.5** `Tests/Integrations/Pdf/PdfDocumentReaderTests.fs` — **Unit:** missing file returns a
      ...
      *Depends on:* 4.4, 4.3.
- [ ] **4.6** `MyDogsbody.Integrations.Pdf/PdfDocumentReader.fs` — PdfPig behind
      ...
      *Depends on:* 4.5.
- [ ] **4.7** Delete `Tests/Integrations/Pdf/Domains/ReadPdfDomainTests.fs`, superseded by 4.5.
      *Depends on:* 4.5, 4.6.

## Phase 5 — Retire Spine (required)

- [ ] **5.1** Repoint `PdfProcessing/Program.fs` at `ReadDocumentLinesWorkflow` with the real
      reader bound to it. Scratch project, but it must compile.
      *Depends on:* 4.6.
- [ ] **5.2** Move the assertions carried by `Tests/Contracts/CredentialHopChainTests.fs` onto their
      ...
      *Depends on:* 6.2, 6.4.
- [ ] **5.3** Delete `Tests/Spine/Domains/DocumentDomainTests.fs`, superseded by 4.2.
      *Depends on:* 4.3.
- [ ] **5.4** Delete `MyDogsbody.Spine/` from disk and from `MyDogsbody.sln`; remove its
      `ProjectReference` from `MyDogsbody.Tests.fsproj` and anywhere else it survives.
      *Depends on:* 5.1, 5.2, 5.3, 3.7.
- [ ] **5.5** Grep for `Domian` across `.fs` and `.fsproj` and confirm no identifier remains.
      *Depends on:* 5.4, 6.1.

## Phase 6 — Action names and contract suites (required)

- [ ] **6.1** Rewrite `MyDogsbody.Exceptions.Types/ActionNames.fs` — nested modules mirroring the
      ...
      *Depends on:* 5.4, 4.6.
- [ ] **6.2** `Tests/Contracts/CredentialBoundaryMapperTests.fs` — both edge mappers field-for-field
      ...
      *Depends on:* 2.4, 3.4.
- [ ] **6.3** `Tests/Contracts/CredentialDependencyContractTests.fs` — one shared suite per
      ...
      *Depends on:* 3.5.
- [ ] **6.4** `Tests/Contracts/PersistedShapeTests.fs` — persisted document field names for
      `Credential` and `ExceptionLog`, asserted by name; every `Infrastructure` member round-trips
      through the store.
      *Depends on:* 2.5, 7.3.
- [ ] **6.5** `Tests/Contracts/ActionNamesTests.fs` — every outer-ring function's error reports its
      declared action string.
      *Depends on:* 6.1.
- [ ] **6.6** `Tests/Contracts/CredentialApiContractTests.fs` — one shared suite over `CredentialApi`,
...
- [ ] **7.1** Add `MyDogsbody.Logging` `ProjectReference` to `MyDogsbody.Tests.fsproj`.
      Resolution 4 in design.md → *Specification conflicts found*.
- [ ] **7.2** `Tests/Logging/ExceptionStoreTests.fs` (Integration) — fresh temp log database per
      ...
      *Depends on:* 7.1.
- [ ] **7.3** Change `ExceptionRepository.insertOne`/`getAll` and
      ...
      *Depends on:* 7.2.
- [ ] **7.4** `Startup.fs` — the `writeLog` callback discards the log write's `Result` with a comment
...
- [ ] **8.1** Add `MyDogsbody.Database` and `MyDogsbody.Database.Migrations` `ProjectReference`s to
      `MyDogsbody.Tests.fsproj`.
- [ ] **8.2** `Tests/Database/MigrationsTests.fs` (Integration) — fresh temp SQLite file per test;
...
- [ ] **9.1** Add bUnit and the MudBlazor test services to `MyDogsbody.Tests.fsproj`; build
      ...
      *Depends on:* 3.5.
- [ ] **9.2** `Tests/E2E/CredentialsFlowTests.fs` — through a rendered component down to a real temp
...
- [ ] **10.1** `dotnet build MyDogsbody.sln` — zero errors, every project including the scratch ones.
- [ ] **10.2** `dotnet test MyDogsbody.Tests\MyDogsbody.Tests.fsproj` — zero failures, **zero skips**.
      *Depends on:* 10.1.
- [ ] **10.3** Run each level filter (`--filter "Level=Unit"`, `Integration`, `Contract`, `E2E`) and
      confirm every level reports a non-zero, passing count.
      *Depends on:* 10.2.
- [ ] **10.4** `dotnet run --project MyDogsbody\MyDogsbody.csproj` — open the window, add, edit and
...
- [ ] **11.1** `CLAUDE-project.md` — project table (add `MyDogsbody.Domain`, drop `MyDogsbody.Spine`),
      ...
      *Depends on:* 10.3.
- [ ] **11.2** Record in this folder the four resolutions from design.md → *Specification conflicts
...
- [ ] **O.1** Collapse the logging project's `ExceptionRepositoryTypeDto` / `ExceptionUseCaseTypeDto`
...
- [ ] **O.2** Rename `GetInfrustructureCredentialCallback` in
...
- [ ] **O.3** Give `CredentialsDatabaseContext` and `LoggingDatabaseContext` a `Dispose`, so
      integration tests stop deleting temp files on a best-effort basis.
- [ ] **O.4** Add a build-time check that `MyDogsbody.Domain.fsproj` stays reference-free, so the
      invariant fails the build rather than a test. 0.5 covers it as a test; this makes it earlier.

## Known risks

- **9.1 may not be achievable as specified.** If MudBlazor cannot be rendered headlessly against
  Fun.Blazor.MudBlazor 8.13.0, the flows drop to the module creator over the real factory, which
  covers everything except the rendered markup. That is a **shortfall to report**, not a level to
  claim — say which level was not met and why in the change description.
- **Phase 3 is the widest single step.** It swaps the whole credentials path in one go, because the
  factory cannot bind half a workflow. 3.6 is what proves it, and Phase 3 must not be left
  incomplete across a session.
- **Phase 6.1 renames every action string.** Any exception rows already in a developer's
  `Logging.db` will carry the old names. The log database holds no domain data and can be deleted
  between runs, so no migration is written for it.
